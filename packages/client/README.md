...
`Orchestration.makeLineup({ filler })` is opt-in scheduling for clips that must play in the order they were written with filler in between, such as a host's spoken lines between idle moments. It enqueues each clip behind the clips already waiting and ahead of all waiting filler, moves it once Ready directly behind the Ready clips, and keeps `filler.ready` filler clips from `filler.clip(n)` Ready behind the playing clip, built one at a time. Clips overtake filler only, never each other or a clip the lineup did not enqueue, and with autoplay on nothing is cut. `enqueue` returns the clip's id, its `fate` and `remove`. The fate is `Started`, with the start's observed time and length; `Failed`, for a clip that will not play (a failed build, a clip lost with its session, one removed with `remove`, even while it was building, or one still waiting when the orchestration ends); or `Unobserved`, for a clip that may have played but whose start the lineup never saw, which carries no start time. `filler.clip(n)` is asked for each n once, in order, and a refused filler is sent again as the same request; a new lineup starts again from zero, so keep `clip` pure. `state` says whether a lineup clip, filler or another clip is playing, how many filler clips are Ready and how often playout ran dry. The lineup knows its own clips by the requests it captured, so it reads no clip metadata. `layerLineup(options)` provides it as the `Lineup` service over the `Engine` in context.
