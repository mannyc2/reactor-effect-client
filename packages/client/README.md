...
`Orchestration.makeScheduler(options)` is an opt-in clip scheduler over the `Engine`; `layerScheduler(options)` provides it as an Effect service. Apps submit stable keys into priority lanes, listed highest first. The `lineup(filler)` preset supplies one `line` lane above generated filler. Each lane is FIFO unless a deadline gives an earlier item priority for build admission. The scheduler admits one build at a time by default, keeps a floor and target of playable Ready seconds, and orders Ready clips within each physical session. Filler is generated by a pure `clip({ index, runwaySeconds, targetSeconds })` callback and is never owed. A replacement session has its own queue and runway during renewal; once an app clip is Ready there, the retiring session's Ready filler is withdrawn so the switch follows its playing clip. A submitted request cannot set `position`, `before`, `sameSessionAs`, `continueFrom` or sequence ownership, because those can override the scheduler's order.

`submit({ key, lane, request, window, start })` returns a handle with `started`, `outcome` and `firstDecisive` effects. `firstDecisive` retains the first observed start or a definitive disposition even when the caller waits after later as-run events; an interim `Unknown` does not settle it. Repeating the same key and spec returns the same handle; a changed spec fails `KeyMismatch`. `window.notBefore` and `window.startBy` are relative monotonic durations, with firm deadlines dropped on expiry and soft deadlines recorded late at start. `start` defaults to `Follow` (the next boundary); `At` names an epoch millisecond anchor and a late policy (`nextBoundary`, `skipIfLaterThan`, or `drop`). The scheduler fills toward an upcoming anchor and never cuts a playing clip. `WouldMissDeadline` rejects an item whose known queued material already makes its deadline impossible. `withdraw(key)` waits for a provider-confirmed removal, including an in-flight build. `drain` stops admission, withdraws waiting clips and waits for the current clip to end; the owner then closes its orchestration handle.

`asRun` is an unbounded per-item lifecycle stream: Accepted, Building, Ready, Started, Ended, Dropped, Failed, Unobserved or Unknown. Each item also has an `outcome` wait. `Started.at` and `Ended.at` are observed epoch times; elapsed airtime and deadline windows use Effect's monotonic clock. `Unobserved` means an acknowledged clip's start was not seen, including after observation overflow. `Unknown` means dispatch has no acknowledgment. A keyed clip observed later can still acquire its Ready or Started evidence. An absent queue snapshot cannot prove an unknown clip never played, so the scheduler never replays that key automatically; it emits `Unknown` with `terminal: true` when the original source retires or the scheduler closes without keyed proof. A withdrawal of that key fails with the original uncertain outcome unless a provider clip is found and removed. The scheduler carries an item key in library-owned provider metadata and leaves the app's request metadata untouched. As-run records provider evidence; an app that needs proof of presented or encoded output must gather that separately.
