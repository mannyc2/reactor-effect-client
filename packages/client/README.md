...
`Orchestration.makeLineup({ filler })` is opt-in scheduling for clips that must play in the order they were written with filler in between, such as a host's spoken lines between idle moments. It enqueues each clip behind the clips already waiting and ahead of all waiting filler, moves it once Ready directly behind the Ready clips, and keeps `filler.ready` filler clips from `filler.clip(n)` Ready behind the playing clip, built one at a time. Clips overtake filler only, never each other or a clip the lineup did not enqueue, and with autoplay on nothing is cut. `enqueue` returns the clip's id, its `fate` (`Started`, with the start's observed time and length, or `Failed`, including a clip lost with its session, removed with `remove`, or still waiting when the orchestration ends) and `remove`. `state` says whether a lineup clip, filler or another clip is playing, how many filler clips are Ready and how often playout ran dry. The lineup knows its own clips by the requests it captured, so it reads no clip metadata. `layerLineup(options)` provides it as the `Lineup` service over the `Engine` in context.
