...
export function unhex(value: string): Uint8Array<ArrayBuffer> {
  ...
  return new Uint8Array(
    Array.from({ length: value.length / 2 }, (_, i) => parseInt(value.slice(i * 2, i * 2 + 2), 16)),
  );
}
...
export function element<A extends HTMLElement>(
  id: string,
  check: (value: HTMLElement) => value is A,
): A {
  const item = document.getElementById(id);
  assert(item !== null && check(item), "missing element " + id);
  return item;
}
/** Container parsing, not decoding. Validate every top-level ISO BMFF length and required boxes. */
export function mp4Boxes(
  bytes: Uint8Array<ArrayBuffer>,
): readonly { type: string; size: number }[] {
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength),
    boxes: { type: string; size: number }[] = [];
  let offset = 0;
  while (offset < bytes.length) {
    ...
    if (size === 1) {
      assert(bytes.length - offset >= 16, "truncated extended box size");
      const big = view.getBigUint64(offset + 8);
      assert(big <= BigInt(Number.MAX_SAFE_INTEGER), "oversize MP4 box");
      size = Number(big);
      header = 16;
    }
  ...
  for (const type of ["ftyp", "moov", "moof", "mdat"])
    assert(
      boxes.some((box) => box.type === type),
      "missing " + type,
    );
  return boxes;
}
