...
| Selected ICE pair types, and once a second: RTT, received kbps, fps, jitter and loss                                                                                                                                                                                                                                                      | Whether TURN is needed; the network expectations in the README                                                                                                                                             | `session.stats`                                      |
...
4. **Whether an attached connection receives media without `resume_track`.** The library resumes tracks only for a session it created, not one it attached to, and the protocol documents `resume_track` for input tracks. The takeover's fresh frames answer it; a takeover that sees none says so in its reason.
...
5. **Rehearsed end to end.** `rehearse <check>` runs the same code against `twin/`, a local stand-in for:
   ...
   - the H3 model's messages, in the documented order: a command's reply before the broadcasts it causes, and autoplay off until a client turns it on;
   - media and stats.
