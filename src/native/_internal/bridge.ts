...
import { ReactorError, ErrorCode } from "../../errors.js";
...
const defaultPaths = (): readonly string[] => {
  ...
  return [
    ...new Set([
      fileURLToPath(new URL(`../${platform}/${name}`, import.meta.url)),
      fileURLToPath(new URL(`../../../dist/native/${platform}/${name}`, import.meta.url)),
    ]),
  ];
};
...
export const resolveNativeBridge = async (path?: string): Promise<string> => {
  ...
  throw new ReactorError(
    "Native",
    "native WebRTC bridge is not staged or its identity is invalid; run scripts/native-build.sh or provide an explicit library path",
