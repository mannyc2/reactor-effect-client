...
export type MemberOutcome =
  | { readonly _tag: "Accepted"; readonly memberId: string; readonly clipId: string; readonly index: number; readonly final: boolean }
  | { readonly _tag: "Rejected"; readonly memberId: string; readonly reason: string | undefined }
  | { readonly _tag: "Indeterminate"; readonly memberId: string; readonly reason: string | undefined }
...
export interface Affinity<Owner> {
  ...
  readonly accepted: (id: string, memberId: string, clipId: string, final?: boolean) => Effect.Effect<void, SequenceError>
  readonly rejected: (id: string, memberId: string, reason?: string, final?: boolean) => Effect.Effect<void, SequenceError>
  readonly uncertain: (id: string, memberId: string, reason?: string) => Effect.Effect<void, SequenceError>
  /**
...
const snapshot = <Owner>(entry: Entry<Owner>): SequenceSnapshot<Owner> => Object.freeze({
  id: entry.id,
  owner: entry.owner,
  status: entry.status,
...
const positiveInteger = (value: number, name: string): Effect.Effect<number, InvalidSequenceOptions> =>
  Number.isSafeInteger(value) && value > 0
    ? Effect.succeed(value)
    : Effect.fail(new InvalidSequenceOptions({ reason: `${name} must be a positive safe integer` }))
...
export const makeAffinity = <Owner>(
  ...
  Effect.gen(function*() {
    ...
    const get = (id: string) => Ref.get(entries).pipe(Effect.map((all) => {
    ...
    const unavailable = (id: string, entry: Entry<Owner>): SequenceError | undefined =>
      entry.status === "sealed" ? new SequenceError({ sequenceId: id, reason: "sealed" })
        : entry.status === "indeterminate" ? new SequenceError({ sequenceId: id, reason: "indeterminate" })
        : entry.status === "retired" ? new SequenceError({ sequenceId: id, reason: "retired" })
        : entry.sealRequested ? new SequenceError({ sequenceId: id, reason: "sealing" })
        : undefined

    const set = (all: ReadonlyMap<string, Entry<Owner>>, entry: Entry<Owner>) =>
      Ref.set(entries, new Map(all).set(entry.id, entry))

    const bind = (id: string, owner: Owner) => lock.withPermit(Effect.gen(function*() {
      const all = yield* Ref.get(entries)
      const existing = all.get(id)
      if (existing !== undefined) {
        if (!sameOwner(existing.owner, owner)) return yield* new SequenceError({ sequenceId: id, reason: "owner-mismatch" })
        const failure = unavailable(id, existing)
        if (failure !== undefined) return yield* failure
        return existing.owner
      }
      if (all.size >= maxEntries) return yield* new SequenceError({ sequenceId: id, reason: "capacity" })
    ...
    const begin = (id: string, memberId: string) => lock.withPermit(Effect.gen(function*() {
      const all = yield* Ref.get(entries)
      const entry = all.get(id)
      if (entry === undefined) return yield* new SequenceError({ sequenceId: id, reason: "missing" })
      const failure = unavailable(id, entry)
      if (failure !== undefined) return yield* failure
      if (entry.pending.has(memberId) || entry.members.some((member) => member.memberId === memberId)) {
    ...
    const settle = (
      ...
      update: (entry: Entry<Owner>) => Pick<Entry<Owner>, "acceptedCount" | "rejectedCount" | "indeterminateCount">,
      ...
      const status: SequenceStatus = entry.status === "indeterminate" || entry.status === "retired"
        ? entry.status
        : terminal === "unknown"
        ? "indeterminate"
        : sealRequested && pending.size === 0 ? "sealed" : entry.status
      yield* set(all, {
        ...entry,
        ...counts,
        status,
    ...
    const accepted = (id: string, memberId: string, clipId: string, final = false) =>
      settle(id, memberId,
        ...
        (entry) => ({ acceptedCount: entry.acceptedCount + 1, rejectedCount: entry.rejectedCount, indeterminateCount: entry.indeterminateCount }),
        "known", final)

    const rejected = (id: string, memberId: string, reason?: string, final = false) =>
      settle(id, memberId,
        () => ({ _tag: "Rejected", memberId, reason }),
        (entry) => ({ acceptedCount: entry.acceptedCount, rejectedCount: entry.rejectedCount + 1, indeterminateCount: entry.indeterminateCount }),
        "known", final)

    const uncertain = (id: string, memberId: string, reason?: string) =>
      settle(id, memberId,
        () => ({ _tag: "Indeterminate", memberId, reason }),
        (entry) => ({ acceptedCount: entry.acceptedCount, rejectedCount: entry.rejectedCount, indeterminateCount: entry.indeterminateCount + 1 }),
        "unknown")

    const seal = (id: string) => lock.withPermit(Effect.gen(function*() {
      const all = yield* Ref.get(entries)
      const entry = all.get(id)
      if (entry === undefined) return yield* new SequenceError({ sequenceId: id, reason: "missing" })
      if (entry.status === "indeterminate") return yield* new SequenceError({ sequenceId: id, reason: "indeterminate" })
      if (entry.status === "retired") return yield* new SequenceError({ sequenceId: id, reason: "retired" })
    ...
    const retire = (owner: Owner) => lock.withPermit(Ref.update(entries, (all) => {
      ...
      for (const [id, entry] of all) {
        ...
        const pendingOutcomes = [...entry.pending].map((memberId): MemberOutcome =>
          Object.freeze({ _tag: "Indeterminate", memberId, reason: "owning session retired before outcome was known" }))
        next.set(id, {
          ...entry,
    ...
    const acknowledgeIndeterminate = (id: string) => lock.withPermit(Effect.gen(function*() {
      const all = yield* Ref.get(entries)
      const entry = all.get(id)
      if (entry === undefined) return yield* new SequenceError({ sequenceId: id, reason: "missing" })
    ...
    const release = (id: string) => lock.withPermit(Effect.gen(function*() {
      const all = yield* Ref.get(entries)
      const entry = all.get(id)
      if (entry === undefined) return yield* new SequenceError({ sequenceId: id, reason: "missing" })
