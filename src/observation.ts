...
interface Entry<A> { readonly value: A; readonly bytes: number }
...
export interface ObservationOptions { readonly capacity?: number; readonly maxBytes?: number }
...
export class Observations<A> {
  ...
  subscribe(options: ObservationOptions = {}): Effect.Effect<Stream.Stream<A, ReactorError>, ReactorError, Scope.Scope> {
    ...
    return Effect.gen(function* () {
      const bounds = yield* Effect.try({
        try: () => ({
          ...
          maxBytes: positiveLimit(options.maxBytes ?? 1_048_576, "observation bytes", 64 * 1024 * 1024),
        }),
        catch: errorOf,
      });
      const subscriber = yield* Effect.acquireRelease(Effect.gen(function* () {
        if (observations.subscribers.size >= observations.maxSubscribers) {
          return yield* Effect.fail(new ReactorError("Overflow", "observer count bound reached", { outcome: "not-submitted" }));
        }
        ...
        if (observations.closed) Queue.endUnsafe(queue);
        else observations.subscribers.add(subscriber);
        return subscriber;
      }), (subscriber) => Effect.gen(function* () {
      ...
      return Stream.unwrap(Effect.gen(function* () {
        yield* Effect.acquireRelease(Effect.try({
          try: () => {
            if (reading) throw new ReactorError("AlreadyReading", "this observation already has an active reader");
            reading = true;
          },
          catch: errorOf,
        }), () => Effect.sync(() => { reading = false; }));
        return Stream.fromEffectRepeat(Effect.uninterruptibleMask((restore) => restore(Queue.take(subscriber.queue)).pipe(
          Effect.map((entry) => {
  ...
  emit(value: A, bytes = 256): void {
    ...
    for (const subscriber of this.subscribers) {
      if (bytes > subscriber.maxBytes - subscriber.bufferedBytes ||
        !Queue.offerUnsafe(subscriber.queue, { value, bytes })) {
        this.overflowCount++;
        Queue.failCauseUnsafe(subscriber.queue, Cause.fail(new ReactorError(
          "Overflow", "observation bound exceeded; acquire a new observation and snapshot",
        )));
