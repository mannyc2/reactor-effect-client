...
import { record, finite } from "./json.js";
...
export const decodeAudioPacket = (input: unknown, maxBytes: number): WebAudioSample => {
  ...
  if (!Number.isSafeInteger(frames) || frames < 1 || frames > 16_384 ||
    !Number.isSafeInteger(sampleRate) || sampleRate < 1 || sampleRate > 384_000) {
    throw new ReactorError("Protocol", "invalid PCM frame count or sample rate");
  }
  const frame = finite(p.frame, "context frame"), skipped = finite(p.skipped, "skipped frames");
  ...
  if (p.planes.length * frames * 4 > maxBytes) throw new ReactorError("Overflow", "PCM sample byte bound exceeded");
  ...
  return Object.freeze({ format: "f32-planar", planes: Object.freeze(planes), frames, sampleRate,
    timestampUs: frame * 1_000_000 / sampleRate, contextFrame: frame, skippedFrames: skipped,
