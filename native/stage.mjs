...
const sources = (directory, prefix = "") => readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
  const relative = `${prefix}${entry.name}`;
  return entry.isDirectory() ? sources(join(directory, entry.name), `${relative}/`) : entry.isFile() ? [relative] : [];
});

const sourceHash = () => {
  const native = join(root, "native");
  const files = ["Cargo.toml", "Cargo.lock", "build.rs", ".cargo/config.toml", "include/reactor_effect_native.h",
    ...sources(join(native, "src"), "src/")].sort();
  const hash = createHash("sha256");
  for (const file of files) hash.update(file).update("\0").update(readFileSync(join(native, file))).update("\0");
...
if (input === undefined || platform === undefined || !(platform in targets)) {
  throw new Error("usage: native/stage.mjs <shared-library> <supported-platform-arch>");
}
...
if (start < 0 || end < 0 || bytes.indexOf(prefix, end + suffix.length) !== -1) {
  throw new Error("native staging requires exactly one embedded source/build identity; rebuild the native library");
}
const build = JSON.parse(bytes.subarray(start + prefix.length, end).toString("utf8"));
if (build.schemaVersion !== 1 || build.abiVersion !== 2 || build.profile !== "release" || build.target !== target) {
  throw new Error("native artifact ABI, release profile or target does not match the requested package platform");
}
const expected = sourceHash();
if (build.sourceSha256 !== expected) {
  throw new Error(`native source identity mismatch: artifact ${build.sourceSha256}; current sources ${expected}; rebuild before staging`);
}
const manifest = { schemaVersion: 1, platform, library, sha256: sha256(bytes), build };
const destination = join(root, "dist", "native", platform, library);
...
console.log(JSON.stringify({ artifact: destination, identity, sha256: manifest.sha256, sourceSha256: expected }));
