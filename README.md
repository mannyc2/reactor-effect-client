...
This is not an official Reactor SDK. Protocol material and native WebRTC dependencies are attributed in [NOTICE](./NOTICE) and in each package's `notices/` directory. Version 0.3.0 of all three packages is npm's `latest`, published with provenance by the [release workflow](./.github/workflows/release.yml) after its sources passed the [hosted qualification](./integration/hosted) against hosted Reactor. It changes the 0.2.0 public API incompatibly, so a `^0.2.0` range does not include it. [CHANGELOG.md](./CHANGELOG.md) lists what each release changes.
