    ...
    await run(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const previous = yield* Native.media(session);
          const old = peers[0];
          ...
          const current = yield* Native.media(session);
    ...
    await run(
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const old = yield* Browser.media(session);
          ...
          const next = yield* Browser.media(session);
