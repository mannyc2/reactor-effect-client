...
test("sequence affinity exposes accepted identities, partial outcomes and explicit sealing", async () => {
  await Effect.runPromise(Effect.gen(function*() {
    ...
    expect(yield* affinity.get("run")).toEqual({
      id: "run", owner: "session-a", status: "sealed", acceptedCount: 2, rejectedCount: 1,
      indeterminateCount: 0, pendingCount: 0, sealRequested: true,
      members: [
...
test("member and sequence bounds are explicit and invalid configuration is typed", async () => {
  const badEntries = await Effect.runPromise(Effect.result(Sequence.makeAffinity({ maxEntries: Number.NaN })))
  expect(Result.isFailure(badEntries) && badEntries.failure._tag).toBe("InvalidSequenceOptions")
  const badMembers = await Effect.runPromise(Effect.result(Sequence.makeAffinity({ maxMembers: 0 })))
  ...
  await Effect.runPromise(Effect.gen(function*() {
    ...
    expect(Result.isFailure(afterResolved) && afterResolved.failure.reason).toBe("member-capacity")
  }))
})
...
test("retiring a session turns pending members into retained unknown evidence", async () => {
  await Effect.runPromise(Effect.gen(function*() {
    ...
    expect(snapshot?.members.at(-1)).toEqual({
      _tag: "Indeterminate", memberId: "second", reason: "owning session retired before outcome was known"
    })
  }))
})
...
test("retiring an already-indeterminate sequence resolves its remaining pending members as unknown", async () => {
  await Effect.runPromise(Effect.gen(function*() {
    ...
    expect(snapshot?.members).toContainEqual({
      _tag: "Indeterminate", memberId: "pending", reason: "owning session retired before outcome was known"
    })
  }))
})
