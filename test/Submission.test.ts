...
test("preparation is inert and repeated/concurrent submit commits once", () => run(Effect.gen(function* () {
  let prepares = 0, executions = 0;
  const entered = yield* Deferred.make<void>();
  const finish = yield* Deferred.make<number>();
  const operation = yield* Submission.make({
    id: "one",
    prepare: Effect.sync(() => { prepares++; return 7; }),
  ...
  const callers = yield* Effect.forEach(Array.from({ length: 20 }), () => Effect.forkChild(operation.submit));
...
test("interruption before commit releases preparation resources and sends nothing", () => run(Effect.gen(function* () {
  let acquired = 0, released = 0, dispatched = 0;
  const entered = yield* Deferred.make<void>();
  ...
  const operation = yield* Submission.make({
    ...
    execute: () => Effect.sync(() => { dispatched++; return "accepted"; }),
  });
...
test("commit registration is atomic with dispatch and a failed hook stays Prepared and retryable", () => run(Effect.gen(function* () {
  let prepares = 0, commits = 0, executions = 0, releases = 0;
  const order: string[] = [];
  const operation = yield* Submission.make({
    id: "commit-hook",
    prepare: Effect.acquireRelease(
      Effect.sync(() => { prepares++; order.push("prepare"); return prepares; }),
      () => Effect.sync(() => { releases++; }),
    ),
    commit: (attempt) => Effect.gen(function* () {
      commits++;
      order.push(`commit-${attempt}`);
      if (attempt === 1) return yield* Effect.fail("binding changed" as const);
    }),
    execute: (attempt) => Effect.sync(() => {
      executions++;
...
test("interruption after commit abandons only the caller and retains eventual completion", () => run(Effect.gen(function* () {
  let released = 0, executions = 0;
  const entered = yield* Deferred.make<void>();
  const response = yield* Deferred.make<string>();
  const operation = yield* Submission.make({
    id: "committed",
    prepare: Effect.acquireRelease(Effect.succeed("input"), () => Effect.sync(() => { released++; })),
...
test("session scope termination joins committed work and releases its resources", async () => {
  ...
  const operation = await Effect.runPromise(Submission.make({
    id: "scope-close",
    prepare: Effect.acquireRelease(Effect.void, () => Effect.sync(() => { released++; })),
...
test("a defective execution is recorded and is never retried", () => run(Effect.gen(function* () {
  let calls = 0, released = 0;
  const operation = yield* Submission.make({
    id: "defect",
    prepare: Effect.acquireRelease(Effect.void, () => Effect.sync(() => { released++; })),
    execute: () => Effect.sync(() => { calls++; throw new Error("deliberate implementation defect"); }),
  });
