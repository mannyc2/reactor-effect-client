...
import { nativePeerTesting } from "../src/native-peer.js";
...
describe("native packet parser", () => {
  test("preserves uint64 video identity and owns BGRA/metadata bytes", () => {
    ...
    const frame = nativePeerTesting.parseVideo({
      header: {
        ...
        dataLength: 4, metadataLength: 3,
        frameId: "18446744073709551615", timestampMicros: "9007199254740993",
      },
      payload: source,
    });
    source.fill(0);
  ...
  test("rejects malformed video/event fields and invalid PCM counts", () => {
    protocol(() => nativePeerTesting.parseVideo(packet({
      type: "video", format: "BGRA", track: "main_video", width: 2, height: 1,
      dataLength: 4, metadataLength: 0, frameId: "1", timestampMicros: "1",
    }, [1, 2, 3, 4])));
    protocol(() => nativePeerTesting.parseVideo(packet({
      type: "video", format: "BGRA", track: "main_video", width: 1, height: 1,
      dataLength: 4, metadataLength: 0, frameId: 1, timestampMicros: "1",
    ...
    protocol(() => nativePeerTesting.parseAudio(packet({
      type: "audio", format: "s16le", track: "main_audio", sampleRate: 48_000,
      channels: 2, samples: 3,
    }, [1, 0, 2, 0, 3, 0])));
  });

  test("decodes signed little-endian PCM and validates snapshot counters", () => {
    const audio = nativePeerTesting.parseAudio(packet({
      type: "audio", format: "s16le", track: "main_audio", sampleRate: 48_000,
      channels: 2, samples: 4,
    }, [1, 0, 0xfe, 0xff, 0x2c, 0x01, 0x70, 0xfe]));
    expect([...audio.samples]).toEqual([1, -2, 300, -400]);

    expect(nativePeerTesting.parseSnapshot({
      closed: false, queuedControl: 1, queuedVideo: 2, queuedAudio: 3, queuedBytes: 4,
      droppedVideo: "18446744073709551615", droppedAudio: "0", pendingRequests: 5,
      deliveredVideo: "6", deliveredAudio: "7",
    })).toEqual(expect.objectContaining({
      droppedVideo: 18446744073709551615n,
      deliveredAudio: 7n,
    }));
    protocol(() => nativePeerTesting.parseSnapshot({
      closed: false, queuedControl: 0, queuedVideo: 0, queuedAudio: 0, queuedBytes: 0,
      droppedVideo: 0, droppedAudio: "0", pendingRequests: 0, deliveredVideo: "0", deliveredAudio: "0",
    }));
  });
