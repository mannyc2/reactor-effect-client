...
| `examples/*`          | private                  | Fullstack examples, each its own workspace; the live channel                               |
...
| `scripts`             | root tooling             | `verify`, `test`, `architecture`, `examples`, `pack` and the isolated-consumer fixtures    |
...
A test whose outcome depends on time passing runs on a `TestClock`, never on the wall clock. A real-time wait makes such a test slow, lets a busy runner fire a deadline early or late, and can only show that nothing happened within the wait. Session tests take a clock from `testClock()` in `packages/client/test/harness.ts`, pass it to each `run`, and move it with `advance` or `within`. The H3 suites run through `test/h3/Clock.ts`: its `run` holds the clock still, and its `runFlowing` moves it a millisecond each time every other ready fiber has had its turn, so windows elapse in order. The orchestration suites use `runClock`. Real time remains only where the timing belongs to something outside Effect: libwebrtc and the far peer, child processes, HTTP servers and the browser.
...
Provider/live validation is separate evidence. Tests using local fakes can prove SDK ownership and scheduling semantics; they cannot prove hosted model behavior, provider billing, TURN relay behavior, or browser/native interoperability. Do not spend money or run paid generation for routine contributions. The hosted qualification in `integration/hosted/qualify.ts` is the one paid check, run only by a maintainer who authorizes its spend: each of its three checks (`vertical`, `takeover`, `turn`) refuses before allocating unless it is given `--i-authorize-paid-sessions`, a budget of at most $0.75 that covers its whole 50 s capped session billed as a full minute at the published rate, a total that the ledger's earlier runs still leave room for, and a new evidence file; it stops at the first unknown outcome or unconfirmed termination and never repeats by itself. CI and `verify` run its offline tests and rehearse every check, and every failure path, against a local twin of hosted Reactor; none of them spends money. [`integration/hosted/README.md`](./integration/hosted/README.md) is the plan: what each spending limit buys, what the checks gather and why, and how the results are shared.
