...
`.github/workflows/release.yml` is the only npm publishing workflow. It is manual-only and must be dispatched from the exact `v<version>` tag with the matching version input and `confirm=publish`; all three packages carry that one version. It rebuilds/qualifies Linux x64 and macOS arm64 native artifacts, packs the three archives, reruns the isolated pack/install smoke, and publishes `reactor-effect-client`, then `reactor-effect-browser`, then `reactor-effect-native` from the validated tarballs.
...
Configure npm Trusted Publishing for each of the three package names against the real repository, workflow filename `release.yml`, and environment name `npm`, with direct `npm publish` allowed. The publish job uses OIDC (`id-token: write`) and no long-lived npm token. Each package's `repository` metadata names this repository and its workspace directory; the workflow fails before native builds if that metadata is absent or different.
