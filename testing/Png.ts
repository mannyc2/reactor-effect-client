...
/** Test-only PNG writing. Both this package's adapter tests and the streamer's
 * broadcast tests need real image bytes; production has a decoder only. */
...
export const dataUri = (bytes: Uint8Array) => `data:image/png;base64,${Buffer.from(bytes).toString("base64")}`
